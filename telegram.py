...
GREEN_PAGE_SIZE = 10
TOTAL_GREEN_TEMPLATES = len(GREEN_EMOJI_PREVIEW_IDS)


def build_green_preview(page: int):
    """Green Emoji shablonlarini sahifalab (Logo/Text bo'limidagi kabi),
    har birini haqiqiy custom_emoji_id orqali (rasm ko'rinishida) ko'rsatadi."""
    start = page * GREEN_PAGE_SIZE + 1
    end = min(start + GREEN_PAGE_SIZE - 1, TOTAL_GREEN_TEMPLATES)
    text = f"🟢 Qaysi Green Emoji shablonni yasaymiz? ({start}-{end} / {TOTAL_GREEN_TEMPLATES})\n\n"
    entities = []
    for n in range(start, end + 1):
        emoji_id = GREEN_EMOJI_PREVIEW_IDS[n - 1]
...
def green_page_keyboard(page: int):
    start = page * GREEN_PAGE_SIZE + 1
    end = min(start + GREEN_PAGE_SIZE - 1, TOTAL_GREEN_TEMPLATES)
    rows = []
    row = []
    for n in range(start, end + 1):
        emoji_id = GREEN_EMOJI_PREVIEW_IDS[n - 1]
        kwargs = {"text": str(n), "callback_data": f"greentpl:{n}"}
        if emoji_id:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️ Oldingi", callback_data=f"greenpage:{page - 1}"))
    if end < TOTAL_GREEN_TEMPLATES:
        nav.append(InlineKeyboardButton(text="Keyingi qatorga o'tish ➡️", callback_data=f"greenpage:{page + 1}"))
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text="⬅️ Bosh menyu", callback_data="backmain")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def _send_green_template_page(message: Message, page: int, edit_callback: CallbackQuery | None = None):
    text, entities = build_green_preview(page)
    kb = green_page_keyboard(page)
...
async def section_green(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await _send_green_template_page(callback.message, 0)


@router.callback_query(F.data.startswith("greenpage:"))
async def green_page_nav(callback: CallbackQuery, state: FSMContext):
    page = int(callback.data.split(":")[1])
    await callback.answer()
    await _send_green_template_page(callback.message, page, edit_callback=callback)


@router.callback_query(F.data.startswith("greentpl:"))
async def green_template_chosen(callback: CallbackQuery, state: FSMContext):
    n = int(callback.data.split(":")[1])
    await state.update_data(template_number=n, kind="green")
    await state.set_state(GreenFlow.waiting_text)
    await callback.answer(f"{n}-shablon tanlandi")
    await callback.message.answer(f"✅ {n}-shablon tanlandi.\n\n✍️ Matn yozing (maksimum {MAX_LEN} ta harf, masalan: Salom):")
...
async def green_got_text(message: Message, state: FSMContext):
    ...
    await message.answer("🎨 Endi matn rangini tanlang:", reply_markup=color_keyboard("greentext"))
...
async def _green_generate_and_stage(message_or_callback_msg: Message, state: FSMContext, text_hex: str):
    """Tanlangan bitta Green Emoji shabloniga matn+rangni qo'shib, natija
    faylini diskka yozadi va paths/nick'ni FSM'ga joylaydi — shu yerdan
    keyin oddiy pack-title/pack-type/to'lov oqimi (Name/Logo bilan bir xil)
    davom etadi."""
    data = await state.get_data()
    text = data.get("green_text", "")
    n = data.get("template_number", 1)
    user_id = message_or_callback_msg.chat.id

    out_dir = os.path.join(os.path.dirname(__file__), "output")
    os.makedirs(out_dir, exist_ok=True)

    template_path = GREEN_EMOJI_TEMPLATE_PATHS[n - 1]
    tgs_bytes, _ = logo_engine.add_text_overlay_to_tgs(template_path, text, text_hex)
    out_path = os.path.join(out_dir, f"{user_id}_green_{n:03d}.tgs")
    with open(out_path, "wb") as f:
        f.write(tgs_bytes)

    nick = _random_nick()
    await state.update_data(paths=[out_path], nick=nick, kind="green")
...
def admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[
        ...
        InlineKeyboardButton(text="💰 Green Emojis narxi", callback_data="adm:price_green", style="primary"),
    ], [
