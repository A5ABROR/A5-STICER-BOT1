...
def add_text_overlay_to_tgs(template_path, text, text_hex, font_path=None, watermark=False):
    """Green Emojis kabi tayyor (Svg Group 0'siz) animatsiyalarga matnni
    pastki qismiga alohida qatlam sifatida qo'shadi. build_tgs_sticker'dan
    farqi: bu yerda ranglangan/qayta bo'yaladigan qolip yo'q, faqat matn
    ustiga qo'shiladi, animatsiyaning o'zi o'zgarishsiz qoladi.
    Returns (tgs_bytes, raw_json_dict)."""
    ...
    svg_text = text_to_svg(text, font_path=font_path)
    subpaths, _ = svg_parse_paths(svg_text)
    # Pastki 22%lik chiziqqa sig'diramiz, chetlardan biroz bo'shliq bilan
    box = (w * 0.06, h * 0.76, w * 0.94, h * 0.96)
    fitted = fit_subpaths_to_box(subpaths, box, padding=0.06)
    text_group = _one_color_group(fitted, text_hex, name="GreenText")

    text_layer = {
        "ddd": 0, "ind": 9001, "ty": 4, "nm": "GreenText",
        "sr": 1, "ip": data.get("ip", 0), "op": data.get("op", 180), "st": 0, "bm": 0,
        "ks": {
    ...
    # Lottie'da ro'yxatdagi BIRINCHI qatlam ENG USTIDA chiziladi, shuning
    # uchun matn animatsiyaning ustida ko'rinishi uchun 0-indeksga qo'yamiz.
    data.setdefault("layers", []).insert(0, text_layer)
