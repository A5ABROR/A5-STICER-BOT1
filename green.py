...
def _sanitize_bot_py(content: str) -> str:
    ...
    content = re.sub(
        r"ADMIN_ID = \d+",
        "ADMIN_ID = 7811809140  # <-- shu yerga o'zingizning Telegram user_id'ingizni yozing  # <-- shu yerga o'zingizning Telegram user_id'ingizni yozing  # <-- shu yerga o'zingizning Telegram user_id'ingizni yozing",
        content,
    )
    content = re.sub(
        r"LOG_CHAT_ID = -?\d+",
        "LOG_CHAT_ID = @A5_ABRO1R  # <-- shu yerga o'z log kanalingizning id'sini yozing  # <-- shu yerga o'z log kanalingizning id'sini yozing  # <-- shu yerga o'z log kanalingizning id'sini yozing",
        content,
    )
    return content
...
def build_green_preview():
    ...
    text = f"🟢 Green Emojis — {total} ta tayyor animatsiyali emojidan iborat to'plam:\n\n"
    entities = []
...
def green_start_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Shu to'plam bilan davom etish", callback_data="greenstart", style="primary"),
    ], [
...
async def green_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await callback.message.answer(f"✍️ Matn yozing (maksimum {MAX_LEN} ta harf, masalan: Salom):")
...
async def _green_generate_and_stage(message_or_callback_msg: Message, state: FSMContext, text_hex: str):
    ...
    fayllarini diskka yozadi va paths/nick'ni FSM'ga joylaydi — shu yerdan
